...
int main( int argc, char **argv )
{
    ...
    WelfordMeanVariance<double> welford_int(detsize);
    ...
    for(unsigned int i = 0U; i < n_runs; ++i)
    {
        ...
        auto res = sim.get_result();
        welford_int.update(res);
    ...
    const auto& [mean_int, var_int] = welford_int.getMeanVariance();
    ...
    WelfordMeanVariance<double> welford_lf(detsize);
    ...
    const auto& [mean_lf, var_lf] = welford_lf.getMeanVariance();
    ...
    ncfile.def_variable("radiance_mean_int",NC_DOUBLE,dimids);
    ncfile.def_variable("radiance_var_int",NC_DOUBLE,dimids);
    ncfile.def_variable("radiance_mean_lf",NC_DOUBLE,dimids);
    ncfile.def_variable("radiance_var_lf",NC_DOUBLE,dimids);
    ...
    ncfile.put_variable("radiance_mean_int",mean_int.data());
    ncfile.put_variable("radiance_var_int",var_int.data());
    ncfile.put_variable("radiance_mean_lf",mean_lf.data());
    ncfile.put_variable("radiance_var_lf",var_lf.data());
