    ...
    for(int cost_point_idx = 0; cost_point_idx < N_COSTHETA; ++cost_point_idx)
    {
        ...
        for(int phi_point_idx = 0; phi_point_idx < N_PHI; ++phi_point_idx)
        {
            ...
            if(fabs(den1) < 1e-4f)
            {
                ...
                X1 = tmp*DIVIDE_C(EXP_C(DIVIDE_C(C_MUT,cost_point)*z1),cost_point);
            }
            else
                X1 = DIVIDE_C(EXP_C(DIVIDE_C(C_MUT,cost_point)*z1) - EXP_C(-C_MUA*l_path+DIVIDE_C(C_MUT,cost_point)*photon->zpos),den1*cost_point);

            if(fabs(den2) < 1e-4f)
            {
                //too small, use expansion
                float tmp = l_path*(1.0f-l_path*den2*(0.5f+l_path*DIVIDE_C(den2,6.0f)));
                Y2 = tmp*DIVIDE_C(EXP_C(DIVIDE_C(C_MUT,cost_point)*(D_SLAB-z1)),-cost_point);
            }
            else
                Y2 = DIVIDE_C(EXP_C(DIVIDE_C(C_MUT,cost_point)*(D_SLAB-z1)) - EXP_C(-C_MUA*l_path+DIVIDE_C(C_MUT,cost_point)*(D_SLAB-photon->zpos)),-den2*cost_point);

            float rfac = DIVIDE_C(r0,1.0f-r0*r0*atten2);
            float c1 = pf1*X1 + rfac*(r0*pf1*atten2*X1-pf2*atten*Y2);
            float c2 = pf2*Y2 + rfac*(r0*pf2*atten2*Y2-pf1*atten*X1);
