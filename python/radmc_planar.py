...
class Hybrid:
    ...
    def run(self):
        ...
        self.N_POINTS_COSTHETA = int(test.cfg["mc_config"]["n_costheta"])
        ...
        # Linear phi sampling
        self.N_POINTS_PHI = int(test.cfg["mc_config"]["n_phi"])
        ...
        platform = cl.get_platforms()[int(self.cfg["opencl_config"]["platform"])]
        devices = [platform.get_devices()[int(device_id)] for device_id in self.cfg["opencl_config"]["devices"]]
        ...
        self.detector = np.zeros(self.N_POINTS_COSTHETA *self.N_POINTS_PHI*self.N_threads, dtype=np.float32)
        ...
        self.detector_loc = np.reshape(self.detector,(self.N_threads,self.N_POINTS_COSTHETA,self.N_POINTS_PHI))
